...
double edge_length_2(double x1, double y1, double x2, double y2) {
...
int sample_edge_2(NumericVector seg_len) {
  double tot_len = 0;
...
// [[Rcpp::export]]
DataFrame grow_polygon_2(DataFrame polygon, int iterations, double noise) {
  ...
  for(int i = 0; i < iterations; i++) {
    ...
    delta = edge_noise_2(last_x, last_y, dx, dy, noise);
    new_x = new_x + delta(0);
    new_y = new_y + delta(1);

    last_len = edge_length_2(last_x, last_y, new_x, new_y);
    next_len = edge_length_2(new_x, new_y, next_x, next_y);
