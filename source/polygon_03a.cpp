...
double edge_length_3(double x1, double y1, double x2, double y2) {
  double len = sqrt(pow(x1 - x2, 2) + pow(y1 - y2, 2));
  return len; 
}

NumericVector edge_noise_3(double x0, double y0, double dx, double dy, double noise) {
  ...
  delta(0) = ux * noise * fabs(dy) / 3;
  delta(1) = uy * noise * fabs(dx) / 3;
  return delta;
}

int sample_edge_3(NumericVector seg_len) {
  double tot_len = 0;
...
// [[Rcpp::export]]
DataFrame grow_polygon_3(DataFrame polygon, int iterations, double noise) {
  ...
  for(int i = 0; i < iterations; i++) {
    ...
    delta = edge_noise_3(last_x, last_y, dx, dy, noise);
    new_x = new_x + delta(0);
    new_y = new_y + delta(1);

    last_len = edge_length_3(last_x, last_y, new_x, new_y);
    next_len = edge_length_3(new_x, new_y, next_x, next_y);
