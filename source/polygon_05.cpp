...
NumericVector edge_noise_1(double x0, double y0, double dx, double dy, double noise) {
  ...
  delta(0) = ux * noise * dy / 5;
  delta(1) = uy * noise * dx / 5;
  return delta;
}

// long, spiky noise
NumericVector edge_noise_2(double x0, double y0, double dx, double dy, double noise) {
  NumericVector delta (2);  
  double ux = R::runif(-1, 1);
...
NumericVector edge_noise(double x0, double y0, double dx, double dy, double noise) {
  ...
  if (u < .85) {
    delta = edge_noise_2(x0, y0, dx, dy, noise);
  } else if (u < .95) {
    delta = edge_noise_1(x0, y0, dx, dy, noise);
  } else {
    delta = edge_noise_3(x0, y0, dx, dy, noise);
  }
  return delta;
}
