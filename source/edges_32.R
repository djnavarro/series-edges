...
art_generator <- function(seed) {
  ...
  reallocate_1 <- function(shape, prob = .975, max_it = 5, noise = .05) {
  ...
  reallocate_2 <- function(shape, max_ratio = 3, max_it = 15, noise = .01) {
    nr <- nrow(shape)
    yy <- shape
    ii <- 0
    while(max(yy$seg_len) > (max_ratio * min(yy$seg_len[-nr]))) {
      if(ii > max_it) break
      x0 <- yy |> dplyr::mutate(.tmp_id = dplyr::row_number() * 2)
      x1 <- x0 |> 
        dplyr::mutate(
      ...
      yy <- dplyr::bind_rows(x0, x1) |> 
        dplyr::arrange(.tmp_id) |> 
        dplyr::mutate(seg_len = edge_length(x, y, vec_rot(x), vec_rot(y))) |> 
        dplyr::slice_max(order_by = seg_len, n = nr) |> 
        dplyr::arrange(.tmp_id) |>         
        dplyr::mutate(seg_len = edge_length(x, y, vec_rot(x), vec_rot(y)))
  ...
  show_polygon <- function(polygon, ...) {
    ggplot2::ggplot(polygon, ggplot2::aes(x, y)) +
        ...
        for(k in 1:10) {
          ...
          for (ll in 1:4) {
            ...
            if (runif(1) < .5) {
              ...
              reallocate_2(max_ratio = 15, noise = .01)
            } else {
              polygons[[ijk]] <- polygons[[ijk]] |> 
              reallocate_1(prob = .95, noise = .05)
            }
          ...
          polygons[[ijk]] <- polygons[[ijk]] |>
            reallocate_1(prob = .5, noise = 0) |> 
            smooth_polygon_l(smoothing = 50) |> 
            dplyr::mutate(id = ijk)
        }
      }
    }
    ...
    max(
      unlist(
        purrr::map_dfr(
          ...
          \(x) {
            tibble::as_tibble(
              colorspace::contrast_ratio(unname(hex_shade), x)
            )
          }
        )
      )
  ...
  while (!palette_ok) {
    ...
    if (max_contrast(hex_shade > 3)) palette_ok <- TRUE
  }
