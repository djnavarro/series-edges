...
smooth_polygon_l <- function(polygon, smoothing = 15) {
...
show_multipolygon <- function(polygon, fill, alpha = .08, ...) {
...
art_generator <- function(seed) {
  ...
  hex_yloc <- sample(c(0, -2, 2), size = n, replace = TRUE)
  bg <- hex_shade[1]
  #hex_shade[1] <- bg
